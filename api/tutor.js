export default async function handler(req, res) {
  ...
  try {
    ...
    if (!pregunta.trim()) return res.status(400).json({ error: "La pregunta está vacía." });

    const systemPrompt = `Eres un tutor educativo dentro de un recurso HTML/SCORM.
...
${contexto || "No se proporcionó contexto específico."}`;
    ...
    if (!response.ok) return res.status(response.status).json({ error: "Gemini respondió con error.", details: data });
    ...
    return res.status(500).json({ error: "Error interno del servidor.", details: String(error?.message || error) });
  }
}
